...
export default async (req) => {
    ...
    try {
        ...
        if (action === "register") {
            if (existingUser) return new Response(JSON.stringify({ error: "이미 존재하는 아이디입니다." }), { status: 409 });
            await store.setJSON(id, { pin, createdAt: Date.now() });
            return new Response(JSON.stringify({ success: true, message: "가입 성공!" }), { status: 200 });
        } 
        
        if (action === "login") {
            if (!existingUser) return new Response(JSON.stringify({ error: "존재하지 않는 아이디입니다." }), { status: 404 });
